...
# Reference: https://github.com/nmmapper/dnsdumpster/blob/master/searchutils.py
...
def get_domain(domain: str) -> str:
    o = urlparse(domain)
    url = o.netloc or o.path

    return url


def read_file(filename: str, mode="rb", ext=None, exclude_comments=None) -> Iterable[str]:
    with open(filename, mode) as f:
        for line in f:
            word:str = line.decode().strip()
            if word == "":
                continue
            if exclude_comments and word.startswith(exclude_comments):
                continue
            if ext:
                yield ext(word)
            else:
